...
describe("parsePublicIntent", () => {
  it("parses valid directive with double quotes", () => {
    const src = `// prodcheck:public-intent reason="Public URL checker"
export async function GET(req: Request) {}`;
  ...
  it("parses valid directive with single quotes", () => {
    const src = `// some code
// prodcheck:public-intent reason='Webhook receiver; auth is Stripe signature'
export async function POST(req: Request) {}`;
  ...
  it("returns malformed when reason is missing", () => {
    const src = `// prodcheck:public-intent
export async function GET(req: Request) {}`;
    const result = parsePublicIntent(src);
  ...
  it("returns malformed when reason is empty", () => {
    const src = `// prodcheck:public-intent reason=""`;
    const result = parsePublicIntent(src);
    expect(result).toHaveProperty("raw");
    expect(result).not.toHaveProperty("reason");
  });
  ...
  it("handles whitespace after //", () => {
    const src = `//   prodcheck:public-intent reason="test"`;
    const result = parsePublicIntent(src);
